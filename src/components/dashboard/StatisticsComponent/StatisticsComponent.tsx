...
const ReportComponent: React.FC<TeamComponentProps> = ({ userId }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const [miningRes, stakingRes, listingRes, miningClosedRes, stakingClosedRes, listingClosedRes] = await Promise.all([
  ...
  return (
    <div className="">
      {loading ? (
        ...
        <>
          <div className='p-[30px] sm:p-0'>
            <div className='flex flex-wrap justify-left sm:justify-between items-center mb-[35px] gap-[14px] sm:gap-[5px]'>
              {buttons.map((button) => (
                <div key={button.label} onClick={() => setActiveButton(button.label)} className={`min-w-[140px] flex gap-[5px] px-[5px] py-[10px] rounded-[5px] ${activeButton === button.label ? 'bg-[#3581FF] shadow-[8px_10px_18.5px_0px_rgba(0,22,58,0.25)]' : ''}`}>
                  <Image
            ...
            {(activeButton === 'Заработано' || activeButton === 'Вложено') && <div className="flex flex-wrap gap-[15px] sm:mr-[-15px] sm:ml-[-15px] sm:justify-start justify-center">
              ...
              <div className='mb-[15px]'>
                <div onClick={() => setActiveTab('staking')} className={`flex items-center gap-[12px] justify-center w-[254px] h-[65px]
            ...
            {activeButton === 'Контракты' && <div className="flex gap-[10px]">
              <div className='w-[255px] h-min'>
              ...
              <div>
                <div className='text-[24px] font-bold uppercase'>реферальная программа</div>
                <div className='flex'>
