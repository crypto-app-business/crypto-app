...
export default function Sidebar({ isSidebarOpen, toggleSidebar, setActiveTab, activeTab }) {
  ...
  return (
    <aside
    className={`fixed inset-0 z-50 text-white flex flex-col transition-all duration-300 ease-in-out ${
      isSidebarOpen
        ? "w-full h-full sm:w-[384px] sm:h-auto rounded-none"
        : "w-[370px] h-max rounded-[15px]"
    }`}
  >
      <nav className="flex-grow text-white w-full rounded-none sm:rounded-[15px]">
          ...
          <ul className="w-max mr-auto ml-auto">
            {links.map((elem, index) => (
              <Link href={elem.href} key={index} onClick={() => { setActiveTab(elem.text); toggleSidebar(); }}>
                <li className={`mb-[17px] font-semibold cursor-pointer flex items-center  space-x-2 transition-all  gap-[20px]
                    ${activeTab === elem.text ? "text-[#3581FF]" : "text-[#00163A]"}
                  `}>
                  <Image
